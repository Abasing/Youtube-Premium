(() => {
  ...
  function renderPremium(svg) {
    if (svg.getAttribute(MARKER) === '1') return;

    const ytdLogo = svg.closest('ytd-logo');
    if (ytdLogo && !ytdLogo.hasAttribute('is-red-logo')) {
      ytdLogo.setAttribute('is-red-logo', '');
    }

    const iconGroup = document.createElementNS(SVG_NS, 'g');
    iconGroup.appendChild(createPath(ICON_BODY_D, '#FF0033'));
    iconGroup.appendChild(createPath(ICON_TRIANGLE_D, 'white'));

    const wordmarkGroup = document.createElementNS(SVG_NS, 'g');
    wordmarkGroup.setAttribute('id', 'youtube-paths');
    for (const d of WORDMARK_PATHS) wordmarkGroup.appendChild(createPath(d));
    ...
    svg.style.setProperty('width', '101px', 'important');
    svg.style.setProperty('height', '20px', 'important');
    svg.textContent = '';
    svg.appendChild(iconGroup);
    svg.appendChild(wordmarkGroup);

    svg.setAttribute(MARKER, '1');
  }

  function isHeaderLogoSvg(svg) {
    const logo = svg.closest('ytd-logo');
    if (!logo || logo.hasAttribute('hidden')) return false;
    const host = svg.closest('ytd-topbar-logo-renderer');
    return host !== null && !!svg.closest('a#logo');
  }

  function scan(scope) {
    if (scope.nodeType !== 1) return;

    if (scope.matches('svg') && isHeaderLogoSvg(scope)) {
      renderPremium(scope);
      return;
    }

    const targets = scope.querySelectorAll
      ? scope.querySelectorAll('ytd-topbar-logo-renderer ytd-logo:not([hidden]) yt-icon svg')
      : [];
    for (const svg of targets) {
      if (isHeaderLogoSvg(svg)) renderPremium(svg);
    }
  }
